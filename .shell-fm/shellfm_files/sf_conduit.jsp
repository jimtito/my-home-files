...
, 
    ...
    if (im) {
        ...
        for(var nd = im.parentNode; nd && nd.nodeName != "BODY"; nd = nd.parentNode) {         
           ...
           // pr = tx.match(/(([\$\£]|eur|ils)(<[^>]+>|&nbsp;|\s)?([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)|((([0-9]+[\,\.])?[0-9]+([\,\.][0-9]+)?)(<[^>]+>|&nbsp;|\s|\s\,\-\s)?(eur|ils))/ig);
            //spsupport.log(pr);
            if (pr && pr.length) {
               // if (pr.length == 1) {
                    return pr[0];
    ...
    return "no price";
        ...
        superfish.b.ip="98.108.209.131";
        superfish.b.userid="chrome00000000001";
        superfish.b.appVersion="9.5.0";
...
superfish.b.bl='baidu.bedandbreakfast.beso.bizrate.booking.crunchyroll.dcinside.facebook.flickr.hotmail.linkedin.lolhehehe.lowpriceshopper.microsoft.msn.myspace.pandora.politico.qq.shopzilla.spardeingeld.westsiderentals.wired.yankodesign.youtube.';
...
superfish.publisher.extractTxt = function(img) {
    ...
    // Validate image
    vi: function(imw, imh) {
        //spsupport.log("Validate inImg" + imw + "    " + imh)
        var h = this.h*1.5;
        if (imh > h) {
            var sp = imw - this.inf + 4, iw = this.iw + this.pd*2;
            //spsupport.log("sp = " + sp + "   iw = " + iw);
            this.itn = parseInt(sp/iw);
            //spsupport.log("1. this.itn = " + this.itn);
            if (this.itn > 1) {
                this.gp = parseInt(sp%iw/2);
                this.sm = 0;
                //spsupport.log("2. this.itn = " + this.itn);
                return this.itn;
            }
            else 
                sp = sp + 10;
            ...
            if (this.itn > 1) {
                this.gp = parseInt(sp%iw/2);
                this.sm = 5;
                // spsupport.log("3. this.itn = " + this.itn);
                return this.itn;
            }
            else {
                this.itn = 0;
