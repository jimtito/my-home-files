...
spsupport.api = {
    ...
    sSrp: function() {
        ...
        sp.pageType = (spsupport.whiteStage.rv ? "PP" : "SRP");
...
//                }
        ...
        if( sS && !sfa.isBLSite( obj )){
            ...
            // spsupport.log("send request for reviews");
            if (superfish.b.rvDt) {
            //request revew WL            
                sfa.jsonpRequest(
                        ...
                        function(){
                            ...
                            if (sfu) {
                                ...
                                if( sp.prodPage.s < 2 && !sp.prodPage.e){
                                    ...
                                    // spsupport.log("Before validate inImg " + im.src);
        ...
        if( ss.validRefState() ){
            ...
            for( var i = 0; i < images.length; i++ ){
                ...
                if (spsupport.whiteStage.rv && imgType) {
                    imgType = (superfish.b.rvi(images[i]) ? imgType : 0);
                    // spsupport.log("after valid: imgType = " + imgType + "  " + images[i].src);
                }                    
                if( imgType ){
                    // spsupport.log("found " + images[i].src);
                    if (sb.icons) {
                        if (! found) {
                            sfa.loadIcons();
...
//                        }
            ...
            if( (sb.suEnabled[1] || superfish.b.inimgSrp) && spsupport.sites.su() && !sp.prodPage.p && !sp.prodPage.s && sp.SRP.p.length ){  
                //sp.prodPage.i ++;
                // spsupport.log("srp " + images[i].src);
                if( superfish.sg ){
