...
-- Modified by M. Fauvell <manelfauvell@gmail.com>
---------------------------------------------------
...
local function worker(format, iface)
    ...
    local f = io.popen("/sbin/iwconfig " .. iface)
    local iw = f:read("*all")
    f:close()
    ...
    -- Check if the hardware is active
    if string.find(iw, 'Tx-Power=off',0,true) then
	return winfo
    else
	winfo["{acti}"] = 1
    end

    -- Check if there are a conection.
    if string.find(iw, "Not-Associated",0,true) then
	return winfo
    else
	winfo["{conn}"] = 1
    end
    ...
    --  Calculate percentage of link quality
    now = tonumber(string.match(iw, "Link Quality[=:]([%d]+)"))
    total = tonumber(string.match(iw, "Link Quality[=:][%d]+%/([%d]*)"))
    winfo["{link}"] = math.floor((now*100)/total) or winfo["{link}"]
